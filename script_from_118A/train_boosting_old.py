...
TREE_ESTIMATOR_RANGE = [50, 100, 150, 200, 250]
...
def boosting_worker(current_n_index, x_train_data, y_train_data, cross_fold_count, algorithm_index):
    print '{}{}'.format('Boosting worker with n index ', current_n_index)
    boosting_worker_model = AdaBoostClassifier(DecisionTreeClassifier(max_depth=1), n_estimators=TREE_ESTIMATOR_RANGE[current_n_index],
...
def boosting_worker_pool_builder(x_train_data, y_train_data, cross_fold_count, algorithm_index):
    ...
    for n_search_index in range(len(TREE_ESTIMATOR_RANGE)):
        sample_result = boosting_pool.apply_async(boosting_worker, (n_search_index, x_train_data, y_train_data,
                                                        cross_fold_count, algorithm_index, ))
...
def search_n_candidate_binary(high_range, low_range, x_train_data, y_train_data, fold, algorithm_index):
    counter = 0
    print ("high_range: "+str(high_range));

    while True:
        counter += 1
        print '{}{}'.format('Boosting worker Binary index ', counter)
        ada_model_high = AdaBoostClassifier(DecisionTreeClassifier(max_depth=1), n_estimators=high_range,
                                             algorithm=ALGORITHM_LIST[algorithm_index])
        cross_score_high = 1 - cross_val_score(ada_model_high, x_train_data, y_train_data, cv=fold).mean()

        boosting_model_low = AdaBoostClassifier(DecisionTreeClassifier(max_depth=1), n_estimators=low_range,
                                             algorithm=ALGORITHM_LIST[algorithm_index])
...
def get_train_test_error(x_train, y_train, x_test, y_test, best_n, algorithm_index):
    final_boosting_model = AdaBoostClassifier(DecisionTreeClassifier(max_depth=1), n_estimators=best_n,
                                             algorithm=ALGORITHM_LIST[algorithm_index])
...
def main():
    ...
    for index in range(len(ALGORITHM_LIST)):
        boosting_pool_result = boosting_worker_pool_builder(x_train, y_train, FOLD_COUNTER, algorithm_index=index)
        ...
        current_boosting_info = 'AdaBoost - feature selector: ' + ALGORITHM_LIST[index]
